<x-app>
    <x-slot:title>Nakliye Kaydını Düzenle - MertNakliyat</x-slot:title>

    <div class="max-w-2xl mx-auto">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-slate-800">Nakliye Kaydını Düzenle</h1>
        ...
        <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            <form action="{{ route('nakliye.update', $operation) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="plate_number" id="plate_number" placeholder="33NCR80"
                               value="{{ old('plate_number', $operation->plate_number) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="supplier_name" id="supplier_name" placeholder="firma adı"
                               value="{{ old('supplier_name', $operation->supplier_name) }}"
                               class="w-full px-3.5 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 text-sm" required>
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="quantity" id="quantity" placeholder="10"
                               value="{{ old('quantity', $operation->quantity) }}"
                               class="w-full px-3.5 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 text-sm" required>
                    ...
                    <div>
                        ...
                        <input type="number" name="freight_price" id="freight_price" placeholder="5000"
                               value="{{ old('freight_price', $operation->freight_price) }}"
                               class="w-full px-3.5 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 text-sm" required>
                    ...
                    <div>
                        ...
                        <input type="text" name="store_code" id="store_code" placeholder="12"
                               value="{{ old('store_code', $operation->store_code) }}"
                               class="w-full px-3.5 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 text-sm" required>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <label class="flex items-center justify-center gap-2 p-3 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50 has-[:checked]:bg-amber-50 has-[:checked]:border-amber-500 has-[:checked]:text-amber-900 transition-all">
                            <input type="radio" name="has_vat" value="0" class="text-amber-600 focus:ring-amber-500" {{ old('has_vat', $operation->has_vat) == 0 ? 'checked' : '' }}>
                            <span class="text-sm font-medium">KDV Hariç (KDV'siz)</span>
                        </label>

                        <label class="flex items-center justify-center gap-2 p-3 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50 has-[:checked]:bg-amber-50 has-[:checked]:border-amber-500 has-[:checked]:text-amber-900 transition-all">
                            <input type="radio" name="has_vat" value="1" class="text-amber-600 focus:ring-amber-500" {{ old('has_vat', $operation->has_vat) == 1 ? 'checked' : '' }}>
                ...
                <!-- Butonlar (Vazgeç & Güncelle) -->
                <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                    <a href="{{ route('nakliye.index') }}"
                       class="px-5 py-2.5 rounded-lg border border-slate-300 text-slate-600 hover:bg-slate-50 font-semibold text-sm transition-colors">
                        Vazgeç
                    </a>
